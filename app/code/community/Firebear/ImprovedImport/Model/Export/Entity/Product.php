...
class Firebear_ImprovedImport_Model_Export_Entity_Product extends Mage_ImportExport_Model_Export_Entity_Product
{
        ...
        while (true) {
            ...
            $configurableProductsCollection->addAttributeToFilter(
                ...
                while ($product = $bundleProductsCollection->fetchItem()) {
                    $bundle = $product->getTypeInstance(true)->getOptionsCollection($product);
                    $bundleData[$product->getId()]  = array();
                    $textFull = '';
                    foreach($bundle as $options) {
                        $dataBundle = $options->getData();
                        //Mage::log($options->getSelectionById($dataBundle['parent_id']));
                        $text = "name=" . $dataBundle['default_title'] .",type=" . $dataBundle['type'] .",required=" . $dataBundle['required'];
                        $textFull .= $this->getFormattedBundleSelections(
                            $text,
                            $product->getTypeInstance()
                            ->getSelectionsCollection([$options->getId()], $product)
                            ->setOrder('position', "ASC")
                            );
                    }
                    $bundleData[$product->getId()][] = array('bundle_values' => rtrim($textFull, "|"));
                }
                ...
                if ($offsetProducts == 1) {
                    ...
                    if ($bundleData) {
                        $headerCols = array_merge($headerCols, array('bundle_values'));
                    }
                ...
                foreach ($dataRows as $productId => &$productData) {
                    foreach ($productData as $storeId => &$dataRow) {
                        ...
                        if (!empty($bundleData[$productId]) && !empty($dataRow['sku'])) {
                            $dataRow = array_merge($dataRow, $bundleData[$productId][0]);
                        }
                        ...
                        if (!empty($bundleData[$productId])) {
                            $additionalRowsCount = max($additionalRowsCount, count($bundleData[$productId]));
                        }
                        if (!empty($rowMultiselects[$productId][$storeId])) {
    ...
    protected function getFormattedBundleSelections($optionValues, $selections)
    {
        $bundleData = '';
